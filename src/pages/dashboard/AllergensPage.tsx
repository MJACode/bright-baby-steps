...
import { UtensilsCrossed, AlertTriangle, Check, Clock, Play } from "lucide-react";
    ...
    <div className="space-y-5">
      <div>
        <h1 className="font-display text-2xl font-bold flex items-center gap-2">
          <UtensilsCrossed className="w-6 h-6 text-destructive" /> Allergen Introduction
        </h1>
        <p className="text-muted-foreground text-sm mt-1">
          {activeChild ? `${activeChild.name}'s allergen protocol` : "Clinically-guided allergen introduction"}
        </p>
          ...
          return (
            <Card key={allergen.id} className={cn("border-0", isComplete ? "bg-success/10" : isStarted ? "bg-warning/10" : "bg-secondary")}>
              ...
              <CardContent className="space-y-2">
                ...
                {activeChild && isStarted && !isComplete && (
                  <Button
                    size="sm"
