...
- AI provider: **Anthropic, PBC**. Six edge functions invoke it: chat, briefing, weekly-insights, parse-voice-log, detect-milestone, generate-speech-class. PrivacyPage § 4 + `/subprocessors` reflect this. **DPA still pending** — § 4 currently uses "we have requested a DPA we expect to confirm…" language; will be rewritten with "we have a written DPA" framing the day the executed PDF is in hand.
